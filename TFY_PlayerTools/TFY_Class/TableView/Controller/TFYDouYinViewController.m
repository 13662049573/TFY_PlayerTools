...
@implementation TFYDouYinViewController
...
- (void)viewDidLoad {
    ...
    self.player.disableGestureTypes = PlayerDisableGestureTypesPan | PlayerDisableGestureTypesPinch;
    ...
    /// 更新另一个控制层的时间
    self.player.playerPlayTimeChanged = ^(id<TFY_PlayerMediaPlayback>  _Nonnull asset, NSTimeInterval currentTime, NSTimeInterval duration) {
        @player_strongify(self)
    ...
    /// 更新另一个控制层的缓冲时间
    self.player.playerBufferTimeChanged = ^(id<TFY_PlayerMediaPlayback>  _Nonnull asset, NSTimeInterval bufferTime) {
        @player_strongify(self)
        if ([self.player.controlView isEqual:self.fullControlView]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    TFYDouYinCell *cell = [tableView dequeueReusableCellWithIdentifier:kIdentifier];
    cell.delegate = self;
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [_tableView registerClass:[TFYDouYinCell class] forCellReuseIdentifier:kIdentifier];
