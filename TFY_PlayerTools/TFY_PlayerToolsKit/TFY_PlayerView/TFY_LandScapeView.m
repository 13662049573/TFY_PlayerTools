...
@interface TFY_LandScapeView ()<TFY_SliderViewDelegate>
/// 顶部工具栏
...
@implementation TFY_LandScapeView
...
- (void)layoutSubviews {
    ...
    min_x = (Player_iPhoneX && self.player.orientationObserver.fullScreenMode == FullScreenModeLandscape) ? 44: 15;
    min_y = (Player_iPhoneX && self.player.orientationObserver.fullScreenMode == FullScreenModeLandscape) ? 15: (Player_iPhoneX ? 40 : 20);
    min_w = 40;
    min_h = 40;
    ...
    min_w = 62;
    min_x = self.bottomToolView.player_width - min_w - ((Player_iPhoneX && self.player.orientationObserver.fullScreenMode == FullScreenModeLandscape) ? 44: min_margin);
    min_y = 0;
    min_h = 30;
    ...
    min_x = (Player_iPhoneX && self.player.orientationObserver.fullScreenMode == FullScreenModeLandscape) ? 50: 18;
    min_y = 0;
    min_w = 40;
    min_h = 40;
...
- (UIButton *)backBtn {
    if (!_backBtn) {
        ...
        [_backBtn setImage:Player_Image(@"arrow_left_white") forState:UIControlStateNormal];
    }
    return _backBtn;
}
...
- (UIButton *)playOrPauseBtn {
    if (!_playOrPauseBtn) {
        ...
        [_playOrPauseBtn setImage:Player_Image(@"bottom_window") forState:UIControlStateNormal];
        [_playOrPauseBtn setImage:Player_Image(@"Stop") forState:UIControlStateSelected];
    }
...
- (TFY_SliderView *)slider {
    if (!_slider) {
        ...
        [_slider setThumbImage:Player_Image(@"thumb_light") forState:UIControlStateNormal];
...
- (UIButton *)lockBtn {
    if (!_lockBtn) {
        ...
        [_lockBtn setImage:Player_Image(@"unlock-nor") forState:UIControlStateNormal];
        [_lockBtn setImage:Player_Image(@"lock-nor") forState:UIControlStateSelected];
    }
    return _lockBtn;
}
