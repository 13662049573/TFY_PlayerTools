 ...
 *  设置请求头，默认为空
 */
static NSDictionary *http_Headers = nil;
/**
 *  设置的返回数据类型
 */
...
+ (void)clearCaches {
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:directoryPath isDirectory:nil]) {
        ...
        if (error) {
            NSLog(@"TFY_Networking清除缓存错误: %@", error);
        } else {
            NSLog(@"TFY_Networking清除缓存确定");
        }
    }
}
...
+ (TFY_URLSessionTask *)tfy_requestWithUrl:(NSString *)url refreshCache:(BOOL)refreshCache isShowHUD:(BOOL)isShowHUD showHUD:(NSString *)statusText httpMedth:(NSUInteger)httpMethod params:(NSDictionary *)params progress:(TFY_DownloadProgress)progress success:(TFY_ResponseSuccess)success fail:(TFY_ResponseFail)fail {
    ...
    if (httpMethod == 1) {
        ...
        session = [manager GET:url parameters:params headers:[self requestHeaderFieldValueDictionary] progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        session = [manager POST:url parameters:params headers:[self requestHeaderFieldValueDictionary] progress:^(NSProgress * _Nonnull uploadProgress) {
            if (progress) {
...
+ (TFY_URLSessionTask *)uploadWithImage:(UIImage *)image url:(NSString *)url filename:(NSString *)filename name:(NSString *)name mimeType:(NSString *)mimeType parameters:(NSDictionary *)parameters progress:(TFY_UploadProgress)progress success:(TFY_ResponseSuccess)success fail:(TFY_ResponseFail)fail {
    ...
    TFY_AppDotNetAPIClient *manager = [self manager];
    TFY_URLSessionTask *session = [manager POST:url parameters:parameters headers:[self requestHeaderFieldValueDictionary] constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
...
//    NSString *returnCode = [NSString stringWithFormat:@"%@",dcit[@"returnCode"]];
//    if ([returnCode isEqualToString:@"2003"]) {
