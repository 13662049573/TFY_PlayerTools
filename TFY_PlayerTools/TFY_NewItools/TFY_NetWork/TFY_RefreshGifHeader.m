...
- (void)prepare
{
    ...
    // 设置普通状态的动画图片
    NSArray *idleImages = [self getRefreshingImageArrayWithStartIndex:0 endIndex:39];
    
    [self setImages:idleImages forState:MJRefreshStateIdle];
    
    // 设置即将刷新状态的动画图片（一松开就会刷新的状态）
    NSArray *refreshingImages = [self getRefreshingImageArrayWithStartIndex:0 endIndex:39];
    
    [self setImages:refreshingImages forState:MJRefreshStatePulling];
    
    NSArray *startImages = [self getRefreshingImageArrayWithStartIndex:0 endIndex:39];
...
- (NSArray *)getRefreshingImageArrayWithStartIndex:(NSInteger)startIndex endIndex:(NSInteger)endIndex {
    ...
    for (NSUInteger i = startIndex; i <= endIndex; i++) {
        NSString *imageName = [NSString stringWithFormat:@"loading_0000%ld", (unsigned long)i];
        UIImage *image = [UIImage imageNamed:imageName];
        UIImage *newImage = [self imageByScalingToSize:CGSizeMake(60, 30) andSourceImage:image];
        if (newImage) {
